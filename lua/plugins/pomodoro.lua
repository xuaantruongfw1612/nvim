...
local function update_timer()
    ...
    if remaining_time <= 0 then
        if not show_overtime then
            ...
            local overtime_duration = sessions[current_session].work_break_cycles[sessions[current_session].current_cycle][2] * 1000 -- Convert to milliseconds
            loop.new_timer():start(overtime_duration, 0, vim.schedule_wrap(function()
                show_overtime = false
                if in_break then
