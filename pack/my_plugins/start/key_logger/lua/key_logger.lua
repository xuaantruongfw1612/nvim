...
local key_history = {}
local buf, win
local config = require("key_logger.config").config

-- Chuyển đổi phím đặc biệt sang ký hiệu tùy chỉnh
local function map_special_key(key)
	return config.special_keys[key] or key
end
...
local function update_key_display(key)
	...
	if #key_history > config.max_keys_display then
	...
	local max_width = vim.o.columns - config.window.width_padding
	...
	local opts = {
		...
		row = vim.o.lines - config.window.row_offset,
		col = config.window.col_offset,
		style = "minimal",
		border = "none",
	}
